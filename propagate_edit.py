...
def get_label_data(args, number_of_images, device):
    if args.label_path is not None:
        points, colors, alpha_channel, label = load_dense_label(args.label_path, use_griddata=(not args.use_splat),
...
def make_visuals(args, dataset, model, device):
    ...
    alpha_channels, colors, edit_image, points, points_normalized = get_label_data(args, input_images.size(0), device)
...
if __name__ == '__main__':
    ...
    # Options for fade-in fade-out video
    ...
    now = datetime.now()
    args.out = args.out + f"/{now.strftime('%Y-%m-%d_%H-%M-%S')}_{image_set_name}"
